...
allObjectsInTheGame::allObjectsInTheGame(sf::RenderWindow& window, int widthMap, int heightMap, int countTurels, int countGhostspawns, int countHealthBonuses, int countBulletsBonuses, int countShieldBonuses, int countMines) :window(&window)
{
    map = new Map(widthMap, heightMap);
    hero = new Player(map->getRandomCenterRooms(), sf::Vector2f(23, 45), "image/soldat.png");
    ...
    for (int i = 0; i < countMines; ++i) {
        Mine* mine = new Mine(map->getRandomCenterRooms(), sf::Vector2f(25, 25), "image/mine.png");
